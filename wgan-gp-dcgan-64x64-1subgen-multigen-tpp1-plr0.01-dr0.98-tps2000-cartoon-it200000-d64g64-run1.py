...
DATASETS =  [#name, ratio, path
            ['celeba',  0.0,home_dir + '/datasets/celebA/'],
            ['cartoon', 1.0,home_dir + '/datasets/cartoonset100k_small/'],
            ]
EXP_ROOT_DIR = 'exp_output7'
COMMENT = 'run1'
MODE = 'wgan-gp' # Valid options are dcgan or wgan-gp
ARCH = 'dcgan' # valid options are dcgan, good_gan, or resnet101
DDIM = 64
#GDIM = 43#43,64,94
GDIM = 64#60,88,128
Z1DIM = 128
...
Z2DIM = N_SUBGEN = 1 #  How many sub-generators in the Generator
...
RELU_GP = False
TRAIN_P_START = 2000
TRAIN_P_PERIOD = 1  # train p every TRAIN_P_PERIOD iters. 0 to disable training p
PLR = 0.01
...
EXPERIMENT_NAME = MODE + '-' + ARCH + '-64x64-{}subgen'.format(N_SUBGEN) + \
    '-{}'.format(GEN_TYPE) + \
    ('-rgp' if RELU_GP else '') + \
...
def GoodGenerator(n_samples, noise=None, name='Generator',dim=GDIM,zd=Z1DIM, nonlinearity=tf.nn.relu):
    if noise is None:
        noise = tf.random_normal([n_samples, zd])

    output = lib.ops.linear.Linear('{}.Input'.format(name), zd, 4*4*8*dim, noise)
    output = tf.reshape(output, [-1, 8*dim, 4, 4])

    output = ResidualBlock('{}.Res1'.format(name), 8*dim, 8*dim, 3, output, resample='up')
    output = ResidualBlock('{}.Res2'.format(name), 8*dim, 4*dim, 3, output, resample='up')
    output = ResidualBlock('{}.Res3'.format(name), 4*dim, 2*dim, 3, output, resample='up')
    output = ResidualBlock('{}.Res4'.format(name), 2*dim, 1*dim, 3, output, resample='up')

    output = Normalize('{}.OutputN'.format(name), [0,2,3], output)
    output = tf.nn.relu(output)
    output = lib.ops.conv2d.Conv2D('{}.Output'.format(name), 1*dim, 3, 3, output)
    output = tf.tanh(output)

    return tf.reshape(output, [-1, OUTPUT_DIM])

def FCGenerator(n_samples, noise=None, name='Generator',zd=Z1DIM,FC_DIM=512):
    if noise is None:
        noise = tf.random_normal([n_samples, zd])

    output = ReLULayer('{}.1'.format(name), zd, FC_DIM, noise)
    output = ReLULayer('{}.2'.format(name), FC_DIM, FC_DIM, output)
    output = ReLULayer('{}.3'.format(name), FC_DIM, FC_DIM, output)
    output = ReLULayer('{}.4'.format(name), FC_DIM, FC_DIM, output)
    output = lib.ops.linear.Linear('{}.Out'.format(name), FC_DIM, OUTPUT_DIM, output)

    output = tf.tanh(output)

    return output

def DCGANGenerator(n_samples, noise=None, name='Generator',zd=Z1DIM, dim=GDIM, bn=True, nonlinearity=tf.nn.relu):
    ...
    output = tf.reshape(output, [-1, 8*dim, 4, 4])
    if bn:
        output = Normalize('{}.BN1'.format(name), [0,2,3], output)
    output = nonlinearity(output)

    output = lib.ops.deconv2d.Deconv2D('{}.2'.format(name), 8*dim, 4*dim, 5, output)
    if bn:
        output = Normalize('{}.BN2'.format(name), [0,2,3], output)
    output = nonlinearity(output)

    output = lib.ops.deconv2d.Deconv2D('{}.3'.format(name), 4*dim, 2*dim, 5, output)
    if bn:
        output = Normalize('{}.BN3'.format(name), [0,2,3], output)
    output = nonlinearity(output)

    output = lib.ops.deconv2d.Deconv2D('{}.4'.format(name), 2*dim, dim, 5, output)
    if bn:
        output = Normalize('{}.BN4'.format(name), [0,2,3], output)
    output = nonlinearity(output)

    output = lib.ops.deconv2d.Deconv2D('{}.5'.format(name), dim, 3, 5, output)
    output = tf.tanh(output)
...
def ResnetGenerator(n_samples, noise=None,name='Generator',zd=Z1DIM, dim=GDIM):
    if noise is None:
        noise = tf.random_normal([n_samples, zd])
    ...
    for i in xrange(6):
        output = BottleneckResidualBlock('{}.4x4_{}'.format(name,i), 8*dim, 8*dim, 3, output, resample=None)
    output = BottleneckResidualBlock('{}.Up1'.format(name), 8*dim, 4*dim, 3, output, resample='up')
    for i in xrange(6):
        output = BottleneckResidualBlock('{}.8x8_{}'.format(name,i), 4*dim, 4*dim, 3, output, resample=None)
    output = BottleneckResidualBlock('{}.Up2'.format(name), 4*dim, 2*dim, 3, output, resample='up')
    for i in xrange(6):
        output = BottleneckResidualBlock('{}.16x16_{}'.format(name,i), 2*dim, 2*dim, 3, output, resample=None)
    output = BottleneckResidualBlock('{}.Up3'.format(name), 2*dim, 1*dim, 3, output, resample='up')
    for i in xrange(6):
        output = BottleneckResidualBlock('{}.32x32_{}'.format(name,i), 1*dim, 1*dim, 3, output, resample=None)
    output = BottleneckResidualBlock('{}.Up4'.format(name), 1*dim, dim/2, 3, output, resample='up')
    for i in xrange(5):
        output = BottleneckResidualBlock('{}.64x64_{}'.format(name,i), dim/2, dim/2, 3, output, resample=None)

    output = lib.ops.conv2d.Conv2D('{}.Out'.format(name), dim/2, 3, 1, output, he_init=False)
    output = tf.tanh(output / 5.)

    return tf.reshape(output, [-1, OUTPUT_DIM])


def MultiplicativeDCGANGenerator(n_samples, noise=None,name='Generator',zd=Z1DIM, dim=GDIM, bn=True):
    if noise is None:
        noise = tf.random_normal([n_samples, zd])

    output = lib.ops.linear.Linear('{}.Input'.format(name), zd, 4*4*8*dim*2, noise)
    output = tf.reshape(output, [-1, 8*dim*2, 4, 4])
    if bn:
        output = Normalize('{}.BN1'.format(name), [0,2,3], output)
    output = pixcnn_gated_nonlinearity(output[:,::2], output[:,1::2])

    output = lib.ops.deconv2d.Deconv2D('{}.2'.format(name), 8*dim, 4*dim*2, 5, output)
    if bn:
        output = Normalize('{}.BN2'.format(name), [0,2,3], output)
    output = pixcnn_gated_nonlinearity(output[:,::2], output[:,1::2])

    output = lib.ops.deconv2d.Deconv2D('{}.3'.format(name), 4*dim, 2*dim*2, 5, output)
    if bn:
        output = Normalize('{}.BN3'.format(name), [0,2,3], output)
    output = pixcnn_gated_nonlinearity(output[:,::2], output[:,1::2])

    output = lib.ops.deconv2d.Deconv2D('{}.4'.format(name), 2*dim, dim*2, 5, output)
    if bn:
        output = Normalize('{}.BN4'.format(name), [0,2,3], output)
    output = pixcnn_gated_nonlinearity(output[:,::2], output[:,1::2])
...
def GoodDiscriminator(inputs, dim=DDIM):
...
def MultiplicativeDCGANDiscriminator(inputs, dim=DDIM, bn=True):
    output = tf.reshape(inputs, [-1, 3, 64, 64])
...
def ResnetDiscriminator(inputs, dim=DDIM):
    output = tf.reshape(inputs, [-1, 3, 64, 64])
...
def DCGANDiscriminator(inputs, dim=DDIM, bn=True, nonlinearity=LeakyReLU):
    output = tf.reshape(inputs, [-1, 3, 64, 64])
...
Generator, Discriminator = DCGANGenerator, DCGANDiscriminator
...
if MODE == 'wgan-gp':
    ...
    disc_cost /= 2.
    gen_train_op = tf.train.AdamOptimizer(learning_rate=2e-4, beta1=0.5).minimize(gen_cost,var_list=gen_params)
    disc_train_op = tf.train.AdamOptimizer(learning_rate=2e-4, beta1=0.5).minimize(disc_cost,var_list=disc_params)
    ...
    for iteration in trange(ITER_START, ITERS):
        ...
        for i in xrange(disc_iters):
            if not TRAIN_P_PERIOD > 0: # if fixed categorical probabilities
                ...
                _data = gen.next()
                _disc_cost, _,F_,disc_fake_ = session.run([disc_cost, disc_train_op,F,disc_fake], feed_dict={real_data_int: _data})
